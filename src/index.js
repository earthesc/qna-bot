...
client.on("interactionCreate", async (interaction) => {
  ...
  if (commandName === "ask") {
    ...
    if (!config || !config.notionIds.length) {
      await interaction.reply({ content: "This server hasn't been set up yet. An admin needs to run /qna-setup first.", ephemeral: true });
      return;
    }
    ...
    try {
      ...
      const answer = await askClaude(question, docsContent, config.name || guild?.name || "this server");
  ...
  if (commandName === "qna-setup") {
    ...
    await interaction.reply({ content: "QNA Bot configured! Linked " + notionIds.length + " Notion page(s). Members can now use /ask.", ephemeral: true });
  }

  if (commandName === "qna-status") {
    const config = getServerConfig(guildId);
    if (!config) {
      await interaction.reply({ content: "This server hasn't been set up yet. Run /qna-setup.", ephemeral: true });
      return;
    }
    const embed = new EmbedBuilder()
      .setColor(0x00d26a)
...
if (!token) { console.error("ERROR: DISCORD_TOKEN is not set."); process.exit(1); }
client.login(token);
